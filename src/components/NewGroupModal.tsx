...
export const NewGroupModal = () => {
  ...
  const { mutateAsync, isPending } = useMutation({
    ...
    onError: (err: any) => toast({ title: "Failed to create group", description: String(err?.message ?? err), variant: "destructive" as any }),
  });
      ...
      <DialogContent>
        ...
        <div className="grid gap-4 py-2">
          ...
          <div className="grid gap-2">
            ...
            <Input id="gname" placeholder="Trip to Rome" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div className="grid gap-2">
            <Label htmlFor="members">Members (comma-separated user IDs)</Label>
            <Input id="members" placeholder="u1,u2,u3" value={members} onChange={(e) => setMembers(e.target.value)} />
          </div>
        </div>
        <DialogFooter>

  ...
  return (
    <main className="container max-w-4xl mx-auto py-10">
      ...
      <Card className="rounded-2xl">
        ...
        <CardContent>
          <Tabs value={tab} onValueChange={(v) => setSp((prev) => { prev.set("tab", v); return prev; })}>
            <TabsList>
                ...
                <ul className="space-y-2">
                  {settlementsQuery.data.map((s, idx) => (
                    <li key={idx} className="text-sm">
                      <span className="font-medium">{s.from.value}</span>
                      <span className="mx-2">→</span>
                      <span className="font-medium">{s.to.value}</span>
                      <span className="mx-2">:</span>
                      <span className="text-muted-foreground">{s.amount.toFixed(2)} {s.currency}</span>
                    </li>
                  ))}
                </ul>
              ) : (
